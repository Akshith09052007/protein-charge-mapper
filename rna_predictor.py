...
def predict_structure(sequence: str, min_loop: int = 3, allow_wobble: bool = True) -> dict:
    ...
    def traceback(i: int, j: int) -> None:
        if i >= j: return
        if dp[i][j] == dp[i + 1][j]: traceback(i + 1, j)
        elif dp[i][j] == dp[i][j - 1]: traceback(i, j - 1)
        elif j - i > min_loop and can_pair(seq[i], seq[j], allow_wobble) and dp[i][j] == dp[i + 1][j - 1] + 1:
            pairs.append((i + 1, j + 1)); traceback(i + 1, j - 1)
        else:
            for k in range(i + 1, j):
                if dp[i][j] == dp[i][k] + dp[k + 1][j]:
                    traceback(i, k); traceback(k + 1, j); return
    if n > 1: traceback(0, n - 1)
    pairs.sort()
    structure = ["."] * n
    for i, j in pairs: structure[i - 1], structure[j - 1] = "(", ")"
    return {"sequence": seq, "length": n, "structure": "".join(structure), "base_pair_count": len(pairs), "base_pairs": [{"position_1": i, "position_2": j, "bases": f"{seq[i-1]}-{seq[j-1]}", "type": "canonical" if (seq[i-1], seq[j-1]) in CANONICAL else "wobble"} for i, j in pairs], "minimum_loop_length": min_loop, "wobble_pairs_enabled": allow_wobble, "algorithm": "Nussinov dynamic programming", "complexity": {"time": "O(n^3)", "space": "O(n^2)"}, "warning": "Educational predictor; it maximizes base-pair count and does not calculate thermodynamic free energy."}
...
def main() -> None:
    ...
    try: result = predict_structure(args.sequence or input("Enter RNA sequence (A, C, G, U): "), args.min_loop, not args.no_wobble)
    ...
    print(f"Sequence : {result['sequence']}\nLength   : {result['length']}\nStructure: {result['structure']}\nPairs    : {result['base_pair_count']}")
    print("Pairs    : " + (", ".join(f"{p['position_1']}-{p['position_2']}" for p in result["base_pairs"]) or "None"))
